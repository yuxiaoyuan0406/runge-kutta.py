...
def plot_and_difference(a1: NamedArray, a2: NamedArray):
    ...
    plt.show(block=False)
    print(
        f'RMSE between {a1.name} and {a2.name} = \n\t{root_mean_square_error(a1, a2)}'
    )


if __name__ == "__main__":
    c_out = NamedArray(np.loadtxt('disp_c.dat',
                                  dtype=np.float64).transpose()[2],
                       name="C data",
                       color='red')
    py_out = NamedArray(np.loadtxt('disp_py.dat',
                                   dtype=np.float64).transpose()[0],
                        name="Python data",
                        color='green')
    for_out = NamedArray(np.loadtxt('disp_for.dat',
